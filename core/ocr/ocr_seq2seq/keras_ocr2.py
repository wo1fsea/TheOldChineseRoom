...
class TextImageGenerator(keras.callbacks.Callback):
    ...
    def __init__(self, monogram_file, bigram_file, minibatch_size,
        ...
        self.ig = ImageGenerator(img_w, img_h)
    ...
    def get_random_string(self):
        ...
        for i in range(length):
            char.append(np.random.choice(list(alphabet)))
        return "".join(char)

    def _get_batch_date(self):
        if K.image_data_format() == 'channels_first':
            X_data = np.ones([self.minibatch_size, 1, self.img_w, self.img_h])
        else:
            X_data = np.ones([self.minibatch_size, self.img_w, self.img_h, 1])

        label = np.ones([self.minibatch_size, 16])
        output_length = np.zeros([self.minibatch_size, 1])
        ...
        for i in range(self.minibatch_size):
            ...
            image = self.ig.generate(text)
            image = image.astype(np.float32) / 255

            if K.image_data_format() == 'channels_first':
                X_data[i, 0, 0:self.img_w, :] = image[0, :, :].T
            else:
                X_data[i, 0:self.img_w, :, 0] = image[0, :, :].T

            t_label = text_to_labels(text)
            label[i, 0:len(t_label)] = t_label
            label_length[i] = len(t_label)
            output_length[i] = self.img_w // self.downsample_factor
